...
class IntakeRepository:
    ...
    async def get_session(self, org: uuid.UUID, session_id: uuid.UUID) -> IntakeSession | None:
        res = await self.s.execute(select(IntakeSession).where(
    ...
    async def list_sessions(self, org: uuid.UUID, patient_id: uuid.UUID | None, conversation_id: uuid.UUID | None, status: str | None, limit=50, offset=0) -> Sequence[IntakeSession]:
        conds = [IntakeSession.org_id==org, IntakeSession.deleted_at.is_(None)]
        if patient_id: conds.append(IntakeSession.patient_id==patient_id)
        if conversation_id: conds.append(IntakeSession.conversation_id==conversation_id)
        if status: conds.append(IntakeSession.status==status)
        res = await self.s.execute(select(IntakeSession).where(and_(*conds)).order_by(IntakeSession.created_at.desc()).limit(limit).offset(offset))
    ...
    async def upsert_chief(self, org: uuid.UUID, session_id: uuid.UUID, text: str, codes: dict | None):
        ...
        res = await self.s.execute(select(IntakeChiefComplaint).where(
    ...
    async def set_social_history(self, org: uuid.UUID, session_id: uuid.UUID, data: dict):
        res = await self.s.execute(select(IntakeSocialHistory).where(
        ...
        if obj: obj.data = data
            ...
            self.s.add(IntakeSocialHistory(org_id=org, session_id=session_id, data=data))
        await self.s.flush()

    async def add_notes(self, org: uuid.UUID, session_id: uuid.UUID, notes: list[dict]):
        for n in notes:
            self.s.add(IntakeNote(org_id=org, session_id=session_id, text=n["text"], visibility=n.get("visibility","internal")))
        await self.s.flush()

    async def set_summary(self, org: uuid.UUID, session_id: uuid.UUID, text: str) -> IntakeSummary:
        res = await self.s.execute(select(IntakeSummary).where(
